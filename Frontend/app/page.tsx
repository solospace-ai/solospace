        ...
        <nav className="flex-1 py-4 px-2 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          {!isSidebarExpanded && (
            <button
              ...
              className="w-full flex items-center justify-center py-2.5 text-neutral-400 hover:text-white hover:bg-neutral-900 rounded-lg transition-all cursor-pointer"
                ...
                {Object.values(sessions).length === 0 ? (
                  ...
                  Object.values(sessions).reverse().map((s) => (
                    <button
                      key={s.id}
                      onClick={() => {
                        loadSessionFromDb(s.id);
                        setWorkspaceState("active");
                        setCurrentTab("chat");
                      }}
                      className={`w-full text-left px-3 py-2 rounded-md text-xs truncate font-medium block cursor-pointer transition-colors ${
            ...
            <div className="absolute inset-0 flex flex-col justify-between overflow-y-auto custom-scrollbar">
              ...
              <div className="w-full max-w-2xl mx-auto px-6 py-12 flex flex-col items-center">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full mt-12 select-none">
                  <div
                    onClick={() => { setIsAutoMode(true); startOrchestration("Help me build a SaaS application with Next.js, Stripe payments, and Clerk authentication"); }}
                  ...
                  >
                    ...
                    <div>
                      <h3 className="text-xs font-semibold text-white">Full-Stack SaaS App</h3>
                      <p className="text-[10px] text-neutral-500 mt-1 truncate">Next.js, Stripe, Auth, DB schema.</p>
                    </div>
                  </div>
                  <div
                    onClick={() => { setIsAutoMode(true); startOrchestration("Write a comprehensive competitive analysis framework for a B2B startup in the AI productivity space"); }}
                    className="border border-[#1f1f1f] bg-[#050505] hover:bg-[#0a0a0a] hover:border-neutral-700 p-4 rounded-xl cursor-pointer transition-all flex flex-col justify-between h-24"
                  >
                    <TrendingUp className="w-4 h-4 text-neutral-500" />
                    <div>
                      <h3 className="text-xs font-semibold text-white">Startup Strategy</h3>
                      <p className="text-[10px] text-neutral-500 mt-1 truncate">Market analysis, positioning, GTM.</p>
                    </div>
                  </div>
                  <div
                    onClick={() => { setIsAutoMode(true); startOrchestration("Explain how to implement JWT authentication with refresh tokens in a Node.js API"); }}
                    className="border border-[#1f1f1f] bg-[#050505] hover:bg-[#0a0a0a] hover:border-neutral-700 p-4 rounded-xl cursor-pointer transition-all flex flex-col justify-between h-24"
                  >
                    <Code className="w-4 h-4 text-neutral-500" />
                    <div>
                      <h3 className="text-xs font-semibold text-white">JWT Auth Guide</h3>
                      <p className="text-[10px] text-neutral-500 mt-1 truncate">Tokens, refresh flow, security.</p>
                    </div>
                  </div>
                </div>
              </div>
