...
const getLayoutedElements = (nodes: Node[], edges: Edge[]) => {
  ...
  const layoutedNodes = nodes.map((node) => {
    ...
    const scaledX = nodeWithPosition.x * scaleX;
    const scaledY = nodeWithPosition.y * scaleY;
    return {
      ...node,
      position: {
        x: scaledX - 128,
  ...
  return (
    <div className="w-full h-full flex-1 relative bg-black">
      ...
      >
        ...
        {/* EchoHouse instructional panel moved to the top-left panel */}
        ...
        {/* Persistent legend and EchoHouse instructions — top left */}
        <Panel position="top-left" className="!left-4 !top-16 select-none z-20 flex flex-col gap-2.5">
          <div className="bg-[#0d0d0d]/80 border border-[#1f1f1f] rounded-lg p-2.5 backdrop-blur-md shadow-xl text-[9px] font-mono text-neutral-600 space-y-1.5 w-64">
          ...
          {isEchoHouseMode && (
            <div className="bg-[#0d0d0d]/80 border border-[#1f1f1f] rounded-lg p-2.5 backdrop-blur-md shadow-xl space-y-3 w-64">
